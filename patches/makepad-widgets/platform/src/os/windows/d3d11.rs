...
impl D3d11Window {
    pub fn new(
        ...
        // choose buffer count: use triple buffering in debug for smoother
        // stepping, double buffering in release to save ~16MB per 4K window.
        let buf_count = if cfg!(debug_assertions) { 3 } else { 2 };
...
impl D3d11Cx {
    pub fn new() -> D3d11Cx {
        unsafe {
            ...
            // pick first adapter for now. a future patch could enumerate
            // adapters by GPU_PREFERENCE_LOW_POWER using DXGI1.4's
            // EnumAdapterByGpuPreference to favour integrated graphics.
            ...
            // note: if the device is idle for a long time, it's safe to drop it
            // and recreate later to return driver heap memory to the OS.  a future
            // patch could call a `release_device` method when all windows are closed.
    ...
    /// Drop and recreate the D3D11 device/context pair.  Useful when the
    /// application is idle for long periods (all windows minimized/closed),
    /// because the driver will release most of its heap memory.  Call this
